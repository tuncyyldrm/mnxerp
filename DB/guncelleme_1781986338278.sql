...
-- =====================================================
-- DB EXPORT (20.06.2026 23:12:18)
-- =====================================================

-- STUB VIEWS

IF OBJECT_ID(N'dbo.vw_CariEkstreDetay', 'V') IS NULL
BEGIN
    EXEC('CREATE VIEW dbo.vw_CariEkstreDetay AS SELECT 1 AS Dummy');
END;
GO

IF OBJECT_ID(N'dbo.V_CariAnalizRaporu', 'V') IS NULL
BEGIN
    EXEC('CREATE VIEW dbo.V_CariAnalizRaporu AS SELECT 1 AS Dummy');
END;
GO

IF OBJECT_ID(N'dbo.vw_FaturaDetayRaporu', 'V') IS NULL
BEGIN
    EXEC('CREATE VIEW dbo.vw_FaturaDetayRaporu AS SELECT 1 AS Dummy');
END;
GO

IF OBJECT_ID(N'dbo.vw_StokListesi', 'V') IS NULL
BEGIN
    EXEC('CREATE VIEW dbo.vw_StokListesi AS SELECT 1 AS Dummy');
END;
GO
-- VIEW DEFINITIONS
...
IF OBJECT_ID('dbo.islemkaydı', 'U') IS NOT NULL
BEGIN
    IF EXISTS (
        SELECT 1 FROM sys.indexes 
        WHERE name = 'IX_IslemKaydi_Covering_Amor' 
        AND object_id = OBJECT_ID('dbo.islemkaydı')
    )
    BEGIN
        DROP INDEX [IX_IslemKaydi_Covering_Amor] ON [dbo].[islemkaydı];
    END;

    CREATE NONCLUSTERED INDEX [IX_IslemKaydi_Covering_Amor]
    ON [dbo].[islemkaydı] (ikid ASC)
    INCLUDE (id_name, belgetarihi, belgesaati, faturanumarası, belgenumarası, islemtipi, BB_TL, AB_TL);
END;
GO

-- IX_Islem_DetayKodu_Bakiye_Optimize
IF OBJECT_ID('dbo.islem', 'U') IS NOT NULL
BEGIN
    IF EXISTS (
        SELECT 1 FROM sys.indexes 
        WHERE name = 'IX_Islem_DetayKodu_Bakiye_Optimize' 
    ...
    ON [dbo].[islem] (detay_kodu ASC, I_DATE ASC, I_TIME ASC)
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM sys.indexes 
        WHERE name = 'IX_Islem_IslemNumarasi_Covering' 
        AND object_id = OBJECT_ID('dbo.islem')
    )
    BEGIN
    ...
    ON [dbo].[islem] (islemnumarası ASC)
...
IF OBJECT_ID('dbo.stok', 'U') IS NOT NULL
BEGIN
    IF EXISTS (
        SELECT 1 FROM sys.indexes 
        WHERE name = 'IX_Stok_Filtreleme_Master' 
    ...
    ON [dbo].[stok] (grubu ASC, kateGOri ASC, tipi ASC)
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM sys.indexes 
        WHERE name = 'IX_Stok_B2B_Search_Optimize' 
        AND object_id = OBJECT_ID('dbo.stok')
    )
    BEGIN
    ...
    ON [dbo].[stok] (urunkodu ASC)
    INCLUDE (urun, urunalt, ureticifirma, grubu, kateGOri, tipi, Raf, fiyatı, OEM, STK_FULL, OEM_0, OEM_1, OEM_2, OEM_3, OEM_4);
END;
GO
